...
export async function getStaticProps(context) {
  const res = await fetch(
    `https://sheet.best/api/sheets/25fd01d4-3922-4123-8d9b-b5e04f474bc8?_limit=12&_offset=0`
  );
  const data = await res.json();
  const count = res.headers.get("x-sheet-rows");
  ...
  return {
    props: { data, count }, // will be passed to the page component as props
  };
}
